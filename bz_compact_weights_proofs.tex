...
rows).  Thus \eqref{eq:T3} is \emph{not} a fixed-coefficient combination of
these families, a bounded negative that sharpens the earlier
codimension-one measurements.  Two structural upgrades are identified and
left open: coefficients rational in $n$ (the certificate shape of every
creative-telescoping identity in this area; the first augmentation test was
row-limited and inconclusive), and \emph{marginal} null weights
$v(n,k)+v(n,l)$ with $\sum_kv(n,k)S_k(n)=0$ for the fibre marginal
$S_k(n)=\sum_lT(n,k,l)$, whose own residue calculus comes from the moment
identities $\sum_l\Res_{y=-l}[y^mR(x,y)]=-[$explicit rational$]$ --- these
are the only mechanisms that can carry the $l$-free nested content
($S_{1,4}(k)$, $S_{2,3}(k)$, \dots) that the target provably contains and
that no per-fibre or iterated-residue functional can produce monomially.
