lf=OpenWrite["/home/ubuntu/fable-episode-2/zeta-math-2/work/r3probe.log"];
...
run[label_,rZ_,qZ_,etZ_]:=Module[{h0Z,hjZ,NN,term,ann,ct,ap,ord,coeffs,D0,cp,t0},
  ...
  ann=TimeConstrained[Annihilator[term,{S[nn],S[tt]}],300,"T"];
  If[ann==="T",log[label," ann TIMECAP"];Return[]];
  log[label," ann #",Length[ann]];
  t0=AbsoluteTime[];
  ct=TimeConstrained[MemoryConstrained[CreativeTelescoping[ann,S[tt]-1,{S[nn]}],10*10^9,"M"],900,"T"];
...
run["r1q13 eta0=27 all1", 1,13, Join[{27},Table[1,{13}]]];
run["r3q7 eta0=4 all1", 3,7, Join[{4},Table[1,{7}]]];
run["r3q7 eta0=6 mix", 3,7, {6,1,1,1,2,2,2,2}];
