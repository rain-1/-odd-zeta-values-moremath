...
static int OF = 1;   /* constant offsets 0..OF */
static int evalAt2(const int *A, const int *a, const int *G, const int *gg, long long n, i128 *out)
{
    long long p[7], q[6];
    for (int j = 0; j < 7; j++) p[j] = (long long)A[j] * n + a[j];
    for (int j = 1; j <= 5; j++) q[j] = (long long)G[j - 1] * n + gg[j - 1];
    q[0] = 0;
    return Qval(p, q, out);
}
static int evalAt(const int *A, const int *G, long long n, i128 *out)
{
    int z7[7] = {0,0,0,0,0,0,0}, z5[5] = {0,0,0,0,0};
    return evalAt2(A, z7, G, z5, n, out);
}
...
int main(int argc, char **argv)
{
    ...
    if (argc > 2) OF = atoi(argv[2]);
    long long T[4] = {1, 61, 52921, 94357501};
    int A[7], G[5], a[7], gg[5];
    long long tried = 0, hits = 0;
    int R = (AMAX + 1) * (OF + 1);   /* per-parameter alphabet: slope 0..AMAX, offset 0..OF */
    long long total = 1; for (int i = 0; i < 12; i++) total *= R;
    for (long long code = 0; code < total; code++) {
        long long c = code;
        for (int i = 0; i < 7; i++) { int d = c % R; c /= R; A[i] = d / (OF + 1); a[i] = d % (OF + 1); }
        for (int i = 0; i < 5; i++) { int d = c % R; c /= R; G[i] = d / (OF + 1); gg[i] = d % (OF + 1); }
        i128 v0, v1, v2, v3;
        if (evalAt2(A, a, G, gg, 0, &v0)) continue;
        if (v0 == 0) continue;
        if (evalAt2(A, a, G, gg, 1, &v1)) continue;
        if (v1 != (i128)T[1] * v0 && -v1 != (i128)T[1] * v0) continue;
        if (evalAt2(A, a, G, gg, 2, &v2)) continue;
        if (v2 != (i128)T[2] * v0 && v2 != -(i128)T[2] * v0) continue;
        tried++;
        if (evalAt2(A, a, G, gg, 3, &v3)) continue;
        if (v3 != (i128)T[3] * v0 && v3 != -(i128)T[3] * v0) continue;
        hits++;
        printf("HIT  p=(%dn+%d,%dn+%d,%dn+%d,%dn+%d,%dn+%d,%dn+%d,%dn+%d) q=(%dn+%d,%dn+%d,%dn+%d,%dn+%d,%dn+%d)\n",
               A[0],a[0],A[1],a[1],A[2],a[2],A[3],a[3],A[4],a[4],A[5],a[5],A[6],a[6],
               G[0],gg[0],G[1],gg[1],G[2],gg[2],G[3],gg[3],G[4],gg[4]);
    }
    printf("AMAX=%d OFFSET<=%d  scanned %lld parameter vectors, matched q0,q1,q2: %lld, matched q3 too: %lld\n",
           AMAX, OF, total, tried, hits);
    return 0;
}
