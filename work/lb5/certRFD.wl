...
lf = OpenWrite[DIR <> "certRFD_" <> ORD <> ".log"];
...
X = AA[2, l] - AA[2, k];
Y = AA[1, k] + 3 BB[1, k];
vt = (HarmonicNumber[n, 3] + 2 AA[3, k] + (1/2) X Y);
...
ann = stage["RFD_ann.m", "D1 ann", ANNCAP, Annihilator[obj, {S[n], S[k], S[l]}]];
...
ct1 = stage["RFD_" <> ORD <> "_ct1.m", "D2 ct1", CT1CAP,
...
gb = stage["RFD_" <> ORD <> "_gb.m", "D3 gb", CT1CAP,
...
If[got =!= $Failed,
   Put[{ann, ct1, gb, got}, DIR <> "RFD_" <> ORD <> "_cert.m"];
