...
CT1A = Environment["CT1A"]; CT1A = If[CT1A === $Failed, -1, ToExpression[CT1A]];
CT1B = Environment["CT1B"]; CT1B = If[CT1B === $Failed, -1, ToExpression[CT1B]];
(* CT2V: third argument of the LAST CreativeTelescoping.  certP.wl passes {},
   which would demand a SCALAR telescoper if RISC does not let Support override
   it -- a silent "none" at every rung.  CT2V=sn passes {S[n]} instead. *)
CT2VE = Environment["CT2V"]; CT2V = If[CT2VE === "sn", {S[n]}, {}];
MEMCAP = Environment["MEMCAP"];
MEMCAP = If[MEMCAP === $Failed, 4000000000, ToExpression[MEMCAP]];
TAG = StringReplace[TAUS, "," -> "_"];
lf = OpenWrite[DIR <> "certP2_" <> TAG <> ".log"];
log[x__] := (WriteString[lf, x, "\n"]; Flush[lf]; Print[x]);
log["START ", DateString[], "  TAUS=", TAUS, " ORD=", ORD, " DMAX=", DMAX,
    " MEMCAP=", MEMCAP, " CT1box=", CT1A, "x", CT1B];
...
(* HoldRest is ESSENTIAL -- see the note in certP.wl and PHASE2_CERTS section 17.5. *)
...
doTau[tau_] := Module[{FF0, ann, ct1, gb, ct2, supp, t0, got},
  ...
  (* CT1A/CT1B >= 0 turn the FIRST elimination into a bounded ansatz
     Support -> {S[n]^i S[V2]^j}, i <= CT1A, j <= CT1B, hence a finite linear
     solve that TERMINATES.  The unconstrained call cannot be interrupted.
     Checkpoint name carries the box so bounded and unbounded runs never mix. *)
  ct1 = stage["P_" <> tau <> "_" <> ORD <> "_ct1" <>
                If[CT1A < 0, "", "_b" <> ToString[CT1A] <> "x" <> ToString[CT1B]] <> ".m",
              tau, "ct1",
              If[CT1A < 0,
                 CreativeTelescoping[ann, S[V1] - 1, {S[n], S[V2]}],
                 CreativeTelescoping[ann, S[V1] - 1, {S[n], S[V2]},
                   Support -> Flatten[Table[S[n]^ii S[V2]^jj,
                                            {ii, 0, CT1A}, {jj, 0, CT1B}]]]]];
  ...
  Do[supp = Table[S[n]^i, {i, 0, d}];
     ...
     ct2 = Quiet[Check[MemoryConstrained[
             CreativeTelescoping[gb, S[V2] - 1, CT2V, Support -> supp], MEMCAP], $Failed]];
