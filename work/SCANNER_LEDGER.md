...
## 14. The t = −1 defect anatomized: period-polynomial, not principal-part (fifteenth arc)

Continuation of §13's cusp-limited apparatus (inline runs,
`eps71.log`, `eps72.log`; exact to q^420, floats valid to n ≈ 300 at
440 digits — the n ≥ 380 rows in the logs are precision-floor noise).

**Location and shape.**  Λ_n alternates in sign perfectly (n = 180–260
checked): the obstruction sits AT t = −1.  |Λ_n| ~ n^{-c}: the defect
is (1+t)^{c-1}·log-type at the cusp value, radius exactly 1.

**Response to the two removal attempts:**
1. **F-side vanishing WORKS, one order per zero.**  The W₁₂-odd
   Eisenstein space is 3-dimensional (O₁ = E₂−12E₂(q¹²),
   O₂ = 2E₂(q²)−6E₂(q⁶), O₃ = 3E₂(q³)−4E₂(q⁴), all parities −1
   measured); cusp-1/2 leading values are in exact ratio 1 : 2 : −1,
   so {G₁ = O₁+O₃, G₂ = O₂+2O₃} vanish at the cusps over t = −1.
   With F = G₁ or G₂: c rises from ≈ 0.85 to ≈ 1.8 (one full order),
   ξ = L(f₆,3)/2 unchanged, d³-integrality and the κ → 3 profile
   unchanged, still no character-split deflation (§13's X-2B verdict
   robust across all F's tried).
2. **Source-side (t+1)^m twisting FAILS.**  S = f*·(t+1)^m (rational,
   parity-odd, poles only at t=∞) leaves c unchanged (≈ 0.85 for both
   m = 1, 2); the limit shifts to new values ξ_m ≠ L(f₆,3)/2 (the
   twisted sources carry different period content; ξ₁ = 0.37178…,
   ξ₂ = 0.37927…, unidentified).

**Interpretation.**  The obstruction is the EICHLER PERIOD defect of
θ⁻³S at the cusp pair (1/2, 1/6) — exactly Sol's warning ("the Eichler
integral can still acquire a polynomial transformation defect...
controlled by critical L-values").  It couples multiplicatively
through F (hence 1) and is REGENERATED by any t-rational twist of the
source (hence 2: θ⁻³((t+1)S) ≠ (t+1)θ⁻³S).  It is NOT a principal-
part/Riemann-Roch obstruction: pole-order linear algebra at the cusp
cannot see it.  Removing it requires the source's twisted period
polynomial at the cusp to vanish — finitely many ℚ-linear conditions
on the source with TRANSCENDENTAL coefficient values (twisted
L-values of f₆) — solvable over ℚ only if those periods admit
ℚ-relations across the source family {f*·tʲ}.

**Fifteenth-arc charter (next):** compute the defect coefficients
β_j of each source f*·tʲ (j ≤ 4) from the measured (−1)ⁿn^{-c} tails
(or exactly, via the weight-4 period polynomial of f₆ twisted to the
1/2-cusp), PSLQ for ℚ-relations, and test whether a rational
combination reaches c = ∞ (exponential decay at base 13.93).  If the
periods are ℚ-independent, the level-12 X-2 cell closes entirely and
the obstruction ledger gains a fourth layer: interior branch —
