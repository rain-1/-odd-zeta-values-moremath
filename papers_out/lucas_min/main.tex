...
congruence for Franel's companion at weight $2$ (Theorem \ref{thm:franel}), and for the
first solution of the Brown--Zudilin family (Theorem \ref{thm:row1}).
...
\begin{definition}
\[
  A(n,k)=\binom nk^2\binom{n+k}k^2,\qquad a_n=\sum_{k=0}^n A(n,k),
\]
\[
  T(n,k,l)=\binom{n+k}n\binom nk^2\binom{n+l}n\binom nl^2\binom{n+k+l}n,
  \qquad Q_n=\sum_{k=0}^n\sum_{l=0}^n T(n,k,l).
\]
\lean{A, apery, T, Q}
\end{definition}

\begin{lemma}\label{lem:digits}
For $r,s<p$ one has $\binom{ap+r}{bp+s}\equiv\binom ab\binom rs\pmod p$; and
$\binom{r+s}s\equiv0\pmod p$ when $s\le r$ and $r+s\ge p$.
\lean{choose\_digits, choose\_carry\_zero}
\end{lemma}

\begin{lemma}\label{lem:Adigits}
For $r,s<p$, \ $A(ap+r,\,bp+s)\equiv\binom ab^2\binom{a+b}b^2A(r,s)\pmod p$; and for
$r,s,t<p$, \ $T(ap+r,\,bp+s,\,cp+t)\equiv T(a,b,c)\,T(r,s,t)\pmod p$.
\lean{A\_digits, T\_digits}
\end{lemma}

Summing Lemma \ref{lem:Adigits} over a full base-$p$ block, the double sum factors:

\begin{theorem}\label{thm:row1}
For $r<p$ and every $a\ge0$,
\[
  a_{ap+r}\equiv a_a\,a_r\pmod p,\qquad Q_{ap+r}\equiv Q_a\,Q_r\pmod p,
\]
and hence $a_n\equiv\prod_i a_{n_i}$, $Q_n\equiv\prod_i Q_{n_i}\pmod p$ for the base-$p$
expansion $n=\sum_in_ip^i$.
\lean{apery\_lucas, Q\_lucas, apery\_lucas\_digits, Q\_lucas\_digits}
\end{theorem}

The statement for $a_n$ is Gessel's \cite{Gessel1982}; the proof formalised here is the
digit-block argument, and it applies verbatim to $Q_n$ because the extra binomial factors
in $T$ satisfy the same carry dichotomy.
...
no \texttt{sorry} and no \texttt{native\_decide}. Every declaration cited above satisfies
\[
